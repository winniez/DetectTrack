...
*/
void bgr2hsv( IplImage* bgr, IplImage *hsv )
{
  IplImage* bgr32f;
...
inline float pixval32f(IplImage* img, int r, int c) //TYS_D 函数转移 原来声明在utils.h 原来定义在utils.cpp
{
  return ( (float*)(img->imageData + img->widthStep*r) )[c];
}

histogram* calc_histogram( IplImage** imgs, int n ) //TYS_C
{
  IplImage* img;
  histogram* histo;
  IplImage* h, * s, * v;
  ...
  //histo = malloc( sizeof(histogram) ); //TYS_B 
  histo = new histogram;
  histo->n = NH*NS + NV;
  hist = histo->histo;
...
*/
inline void setpix32f(IplImage* img, int r, int c, float val) //TYS_D 函数转移 原来声明在utils.h 原来定义在utils.cpp
{
  ( (float*)(img->imageData + img->widthStep*r) )[c] = val;
}

IplImage* likelihood_image( IplImage* img, int w, int h, histogram* ref_histo ) //TYS_C
{
  IplImage* l, *l2;
  CvScalar sum;
  int i, j;
