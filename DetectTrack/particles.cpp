...
/*
  ...
  @return Returns an array of \a p particles sampled from around regions in
    \a regions
*/
particle* init_distribution(CvRect region, histogram* histos, int p)
{
	particle* particles;
	int np;
	float x, y;
	...
	particles = new particle[ p ];//TYS_B 
	np = p;
...
particle* resample( particle* particles, int n )
{
  ...
  qsort( particles, n, sizeof( particle ), &particle_cmp );//TYS_B
  //new_particles = malloc( n * sizeof( particle ) );
  new_particles = new particle[ n ];//TYS_B
  for( i = 0; i < n; i++ )
    {
...
int particle_cmp( const void* p1, const void* p2 ) //TYS_B
{
  //particle* _p1 = (particle*)p1;
  //particle* _p2 = (particle*)p2;
  const particle* _p1 = (particle*)p1;
  const particle* _p2 = (particle*)p2;
