...
/*inline float pixval32f(IplImage* img, int r, int c) //TYS_D 已转移到observation.cpp
{
...
/*inline void setpix32f(IplImage* img, int r, int c, float val) //TYS_D 已转移到observation.cpp
{
...
{
  //char* new_file = malloc( (strlen(file)+strlen(new_extn)+2) * sizeof(char) );//TYS_B
  char* new_file = new char[ (strlen(file)+strlen(new_extn)+2) * sizeof(char) ];
...
char* prepend_path(const char* path, const char* file)
{
  ...
  _snprintf( pathname, name_length, "%s/%s", path, file ); //TYS_B //snprintf->_snprintf
